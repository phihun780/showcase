            ...
            <div className="flex items-center gap-0.5 sm:gap-1 bg-[#1e1e1e] p-0.5 rounded border border-[#3e3e3e]">
              {ARTWORKS.map((art) => {
                ...
                return (
                  ...
                  >
                    {/* Điện thoại chỉ để icon cho đỡ chật — ba tab kèm chữ thì
                        thanh công cụ giả này tràn mất. Tên đầy đủ vẫn nằm ở
                        `title`/`aria-label` nên trình đọc màn hình vẫn đọc được.

                        Trước đây dùng `xs:` — mà `xs` không phải breakpoint có
                        thật trong dự án này (không khai báo ở đâu cả), nên hai
                        class đó không sinh ra CSS: tên ngắn hiện ở MỌI cỡ màn. */}
                    <span>{art.emoji}</span>
                    <span className="hidden sm:inline">{art.shortName}</span>
                  </button>
                );
              })}
            </div>
